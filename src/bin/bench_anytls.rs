...
async fn main() -> anyhow::Result<()> {
    ...
    match command {
        ...
        Command::Client(options) => run_client(options).await,
    }
}
...
async fn run_client(options: ClientOptions) -> anyhow::Result<()> {
    let started = Instant::now();
    let mut tasks = Vec::with_capacity(options.parallel);
    for worker_id in 0..options.parallel {
        let options = options.clone();
        tasks.push(tokio::spawn(async move {
            run_worker(worker_id as u32 + 1, options).await
        }));
    }
    ...
    println!(
        "mode={} parallel={} duration={}s uploaded={} MiB ({:.2} Mbps) downloaded={} MiB ({:.2} Mbps)",
        match options.mode {
            BenchMode::Upload => "upload",
            BenchMode::Download => "download",
        },
        options.parallel,
        options.duration.as_secs(),
        mib(summary.uploaded),
        mbps(summary.uploaded, elapsed),
        mib(summary.downloaded),
        mbps(summary.downloaded, elapsed),
    );
    if let Some(value) = average_ms(&connect_values) {
        println!("avg tcp connect: {:.2} ms", value);
    }
    if let Some(value) = average_ms(&handshake_values) {
        println!("avg tls handshake: {:.2} ms", value);
    }
    if let Some(value) = average_ms(&synack_values) {
        println!("avg stream synack: {:.2} ms", value);
    }
    if let Some(value) = average_ms(&first_byte_values) {
        println!("avg first byte: {:.2} ms", value);
    }
    Ok(())
}

async fn run_worker(stream_id: u32, options: ClientOptions) -> anyhow::Result<WorkerResult> {
    ...
    send_auth_preface(&writer, &options.user).await?;
    ...
    let reader_task =
        tokio::spawn(async move { read_loop(reader, reader_writer, reader_state).await });

    let syn_payload = encode_target(&options.target)?;
    *state
        .syn_sent_at
        .lock()
...
async fn read_loop(
    mut reader: ReadHalf<tokio_rustls::client::TlsStream<TcpStream>>,
    writer: Arc<Mutex<WriteHalf<tokio_rustls::client::TlsStream<TcpStream>>>>,
    ...
    loop {
        ...
        match cmd {
            CMD_PSH if stream_id == state.stream_id => state.record_download(length),
...
fn parse_args(args: Vec<String>) -> anyhow::Result<Command> {
    ...
    let Some(command) = args.next() else {
        bail!("usage: bench_anytls <sink|source|client> ...");
    };
...
fn parse_client_args(args: Vec<String>) -> anyhow::Result<ClientOptions> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--user" => user = Some(next_value(&mut iter, "--user")?),
            ...
            "--seconds" => {
                duration = next_value(&mut iter, "--seconds")?.parse::<u64>()?;
            }
            "--parallel" => {
                parallel = next_value(&mut iter, "--parallel")?.parse::<usize>()?;
            }
            "--chunk-size" => {
    ...
    Ok(ClientOptions {
        ...
        user: user.ok_or_else(|| anyhow::anyhow!("missing --user"))?,
        target: target.ok_or_else(|| anyhow::anyhow!("missing --target"))?,
        mode: mode.ok_or_else(|| anyhow::anyhow!("missing --mode"))?,
        duration: Duration::from_secs(duration.max(1)),
        parallel: parallel.max(1),
        chunk_size: chunk_size.clamp(1024, MAX_FRAME_PAYLOAD_LEN),
        insecure,
    })
}
